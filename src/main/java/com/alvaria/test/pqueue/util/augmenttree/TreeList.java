...
/**
 * Implements a list using a self-balancing binary search tree augmented by subtree size.  The benefit of this compared to ArrayList or LinkedList is that it supports both decent random access and
 * quickly adding to or removing from the middle of the list.  Operations have the following running times:
 * <p>
 * size(): O(1) get, set, add, remove: O(log N) addAll: O(log N + P), where P is the number of elements we add iterator(): O(log N + P + M log N), where P is the number of elements over which we
 * iterate and M is the number of elements we remove listIterator: O(log N + P + M log N + R log N), where P is the number of times we iterate over or set an element, M is the number of elements we
 * add or remove, and R is the number of times we change the direction of iteration clear(): O(1), excluding garbage collection subList.clear(): O(log N), excluding garbage collection Constructor:
 * O(N)
 * <p>
 * This class is similar to an Apache Commons Collections class by the same name.  I speculate that the Apache class is faster than this (by a constant factor) for most operations.  However, this
 * class's implementations of addAll and subList.clear() are asymptotically faster than the Apache class's implementations.
 */
public class TreeList<T> extends AbstractList<T> {
  ...
  private TreeListNode<T> createTree(Collection<? extends T> values) {
    List<TreeListNode<T>> nodes = new ArrayList<>(values.size());
    for (T value : values) {
  ...
  private class TreeListIterator implements Iterator<T> {
    ...
    /**
     * The node containing the last element next() returned.  This is null if we have yet to call next() or we have called remove() since the last call to next().
     */
  ...
  private class TreeListListIterator implements ListIterator<T> {
    ...
    /**
     * Whether we have (successfully) called remove() or "add" since the last (successful) call to next() or previous().
     */
     ...
     *
     * @param index The starting index, as in the "index" argument to listIterator.  This method assumes that 0 <= index < root.size.
     */
